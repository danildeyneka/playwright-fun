...
export async function run(isZarplataRu?: boolean) {
	...
	async function main() {
		checkEnv(isZarplataRu);
		
		const browser = await chromium.launch({ headless: true });
		const context = await browser.newContext({
			storageState: (isZarplataRu ? 'zp' : 'hh') + '-state.json',
			viewport: { width: 1366, height: 768 }
		});
		const page = await context.newPage();
		
		// Проверка авторизации
		await checkAuth(page, browser, isZarplataRu);
		
		// максимально откликнуться можно 200 раз, парсим страницы с учетом вакансий-опросников
		for (let i = 1; i < 4; i++) {
			const searchUrl = `${isZarplataRu ? VACANCIES_LIST_ZARPLATA : VACANCIES_LIST}&items_on_page=100${!AUTO_HIDE_VACANCY ? ('&page=' + i) : ''}`;
	...
	async function exit() {
		...
		if (manualVacancies.length) {
			...
			const outputPath = path.join(process.cwd(), 'manual', `${isZarplataRu ? 'zp' : 'hh'}-list-${date}.json`);
